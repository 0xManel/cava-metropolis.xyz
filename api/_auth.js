...
const DEFAULT_FOURTH_ADMINMASTER_USERNAME = 'Diego';
const DEFAULT_FOURTH_ADMINMASTER_PASSWORD_HASH = 'e1df66b36ecf8ecd2e509530e83e1dc0b7fe5446d56efec4c46bca85826caa45';
const DEFAULT_FIFTH_ADMINMASTER_USERNAME = 'Edu';
const DEFAULT_FIFTH_ADMINMASTER_PASSWORD_HASH = '8bb999942ea3670fcb942033d778b34a148e111011b12ea7a1047f472b7b00be'; // sha256("090504")
const DEFAULT_SPA_ADMIN_USERNAME = 'Yeray01';
const DEFAULT_SPA_ADMIN_PASSWORD_HASH = 'b79ea17b7c5ca8fe9cccd8cdba6e8f8ed0b3c948f9f709ed0f47d2fd47fcba82'; // sha256("010203")
...
function getFourthAdminmasterUsername() {
  return String(
    process.env.FOURTH_ADMINMASTER_USERNAME
    || process.env.DIEGO_ADMINMASTER_USERNAME
    || DEFAULT_FOURTH_ADMINMASTER_USERNAME
  ).trim() || DEFAULT_FOURTH_ADMINMASTER_USERNAME;
}

function getFourthAdminmasterPasswordHash() {
  const configured = String(
    process.env.FOURTH_ADMINMASTER_PASSWORD_HASH
    || process.env.DIEGO_ADMINMASTER_PASSWORD_HASH
    || ''
  ).trim().toLowerCase();
  if (/^[a-f0-9]{64}$/.test(configured)) return configured;
  return DEFAULT_FOURTH_ADMINMASTER_PASSWORD_HASH;
}

function getFifthAdminmasterUsername() {
  return String(
    process.env.FIFTH_ADMINMASTER_USERNAME
    || process.env.EDU_ADMINMASTER_USERNAME
    || DEFAULT_FIFTH_ADMINMASTER_USERNAME
  ).trim() || DEFAULT_FIFTH_ADMINMASTER_USERNAME;
}

function getFifthAdminmasterPasswordHash() {
  const configured = String(
    process.env.FIFTH_ADMINMASTER_PASSWORD_HASH
    || process.env.EDU_ADMINMASTER_PASSWORD_HASH
    || ''
  ).trim().toLowerCase();
  if (/^[a-f0-9]{64}$/.test(configured)) return configured;
  return DEFAULT_FIFTH_ADMINMASTER_PASSWORD_HASH;
}
...
function getProtectedAdminmasterUsers() {
  ...
  const seeds = [
    ...
    { username: getFourthAdminmasterUsername(), password_hash: getFourthAdminmasterPasswordHash() },
    { username: getFifthAdminmasterUsername(), password_hash: getFifthAdminmasterPasswordHash() }
  ];
  const seen = new Set();
...
function ensureSeedUsers(users) {
  ...
  const seededUsers = [
    {
      username: DEFAULT_SPA_ADMIN_USERNAME,
      password_hash: DEFAULT_SPA_ADMIN_PASSWORD_HASH,
      role: 'admin',
      scope: 'spa'
    }
  ];
  ...
  seededUsers.forEach((seedUser) => {
    const key = normalizeUsername(seedUser.username);
    const exists = list.some((entry) => normalizeUsername(entry.username) === key);
    if (exists) return;
    list.push(seedUser);
  });
...
async function loadUsers() {
  ...
  if (typeof rawValue === 'string') {
    try {
      ...
      const safeUsers = ensureSeedUsers(ensureOwnerUser(parsedList));
  ...
  if (Array.isArray(globalThis.__cavaAuthUsersMemoryState) && globalThis.__cavaAuthUsersMemoryState.length > 0) {
    return ensureSeedUsers(ensureOwnerUser(globalThis.__cavaAuthUsersMemoryState));
  }

  const defaults = ensureSeedUsers(ensureOwnerUser([]));
  globalThis.__cavaAuthUsersMemoryState = defaults;
  return defaults;
}

async function saveUsers(users) {
  const safeUsers = ensureSeedUsers(ensureOwnerUser(users));
  const previousUsers = ensureSeedUsers(ensureOwnerUser(globalThis.__cavaAuthUsersMemoryState));
